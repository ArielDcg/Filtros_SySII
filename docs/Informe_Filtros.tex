...
\title{\textbf{\Huge Informe de Diseño e Implementación de Filtros} \\
...
\author{
    ...
    \texttt{correo1@institucion.edu}
    \hspace{1.5cm}
    \textbf{Autor 2}\\
    Nombre Completo del Autor 2\\
    \texttt{correo2@institucion.edu}\\
    \vspace{1.5cm}
    \textbf{Autor 3}\\
    Nombre Completo del Autor 3\\
    \texttt{correo3@institucion.edu}
}
...
\begin{itemize}
    ...
    \item \textbf{Filtros Digitales IIR}: Sistemas discretos con retroalimentación (respuesta infinita).
    \item \textbf{Filtros Digitales FIR}: Sistemas discretos sin retroalimentación (respuesta finita).
\end{itemize}
...
Un filtro analógico se define mediante su función de transferencia $H(s)$ en el dominio de Laplace:

\begin{equation}
H(s) = \frac{N(s)}{D(s)} = \frac{\sum_{i=0}^{M} b_i s^i}{\sum_{i=0}^{N} a_i s^i}
\end{equation}

donde $N(s)$ es el polinomio del numerador (ceros) y $D(s)$ es el polinomio del denominador (polos).

La respuesta en frecuencia se obtiene evaluando $H(j\omega)$ donde $j = \sqrt{-1}$ y $\omega$ es la frecuencia angular en rad/s.

La \textbf{magnitud} en decibelios se expresa como:
\begin{equation}
...
La estabilidad requiere que todos los polos estén dentro del círculo unitario: $|z_p| < 1$.

La respuesta en frecuencia se obtiene evaluando $H(e^{j\omega})$, donde $\omega \in [0, \pi]$.

\subsection{Filtros Digitales FIR}
...
Los coeficientes $h[n]$ son simplemente los valores de la respuesta al impulso. Los filtros FIR siempre son
estables porque no tienen polos (excepto en el origen).
